...
Result generated by Adapter must be of type `DataSourceAdapterResponse` with `clientResponse` field set:
 ...
 - status code - http status code of processing. It is available from template under key `_response.statusCode`. Please check [Fallback mechanism](#Fallback mechanism)
